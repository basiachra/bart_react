...
export class Gallery extends React.Component {
    ...
    cardItem = (categories, url, index) => {
       ...
       /* if(count === 1) count = " 1 fotka"
        ...
        return <ContactItem name={categories.name} path={categories.path} key={index} type='photo'  img={url} />;
    };
    ...
    cardItem2 = (url, name, index) => {
        ...
        return <ContactItem  name={name} key={index} type='gallery' img={path}   />;
    };
...
export const ContactItem = ({ name, type, img, path},key) => {
    if (type === 'photo') {
        return (
